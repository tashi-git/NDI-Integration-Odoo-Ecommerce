...
Request the following credentials from the NDI organization:
...
Use the client_id and client_secret with the NDI Authenticator Service to generate an access token.

This token must be included in all API requests.

Authorization: Bearer <access_token>
3. Expose Local Development Server

Expose your local server so that NDI services can access your webhook endpoint.
...
Create a webhook endpoint in your website to receive verification responses from the NDI service.

Example endpoint:

https://your-domain.com/ndi/webhook
5. Register Webhook with NDI

Register your webhook endpoint with the NDI Webhook Service so the system can receive proof verification events.

6. Create Proof Request

Use the NDI Verifier Service to create a proof request.
...
7. Subscribe Webhook to Thread ID

Each proof request has a unique Thread ID.
...
ndi.schema_email	Schema used to retrieve email attribute
